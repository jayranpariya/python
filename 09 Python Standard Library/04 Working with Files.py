...
path = Path(
    r"mosh-complete-python-course-master/09 Python Standard Library\test_folder\test_file.py")
print(path.exists())
# path = path.rename(r"09 Python Standard Library\test_folder\test_file2.py") # To rename a direcdtory
# With the ".stat()" method we get a "os.stat_result" object with the following attributes
print(path.stat())
...
# which return the content of a file as a bytes object for reprersenting binary data
path.read_bytes()

# Which return the content of the file as a string. Using this method it is easier to read a file that the built-in "open()" function.
print(path.read_text())

file = open(
    r"mosh-complete-python-course-master/09 Python Standard Library\test_folder\test_file.py", "r")
print(file.read())
file.close()

# Or use the "with" statment, as a best practice. Because it will automatacly close the file after it finishes the operation

with open(r"mosh-complete-python-course-master/09 Python Standard Library\test_folder\test_file.py", "r") as file:
    print(file.read())
